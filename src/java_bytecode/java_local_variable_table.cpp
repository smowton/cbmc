    ...
    public graph<cfg_base_nodet<T, unsigned> >
{
  ...
  void operator()(const method_with_amapt& args)
  {
    ...
    for(const auto& table_entry : method.exception_table)
    {
      ...
      assert(findit!=amap.end() && "Exception table entry doesn't point to an instruction?");
      for(;findit->first<table_entry.end_pc; ++findit)
      {
        ...
        if(thisinst.successors.size()==1 && *thisinst.successors.begin()==succit->first)
          this->add_edge(entry_map.at(findit->first),entry_map.at(table_entry.handler_pc));
      }
    }
  }
...
java_bytecode_convert_methodt::find_variable_for_slot(
  size_t address, variablest &var_list)
{
