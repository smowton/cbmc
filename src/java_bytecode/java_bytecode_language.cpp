  ...
  symbol_tablet &symtab)
{
  recording_symbol_tablet symbol_table=
    recording_symbol_tablet::wrap(symtab);
  symbolt &symbol=*symbol_table.get_writeable(function_id);

  // Fill body of the function with code generated by string preprocess
  symbol.value=
    string_preprocess.code_for_function(function_id, symbol, symbol_table);
