  ...
  remove_exceptions_typest type)
{
  ...
  Forall_goto_program_instructions(instr_it, goto_program)
  {
    ...
    {
      instrument_throw(function_id, goto_program, instr_it, stack_catch, locals);
    }
    else if(instr_it->type==FUNCTION_CALL)
    {
      instrument_function_call(function_id, goto_program, instr_it, stack_catch, locals);
    }
  }
  if(has_flag(type, remove_exceptions_typest::also_remove_instanceof))
    remove_instanceof(function, symbol_table);
  else if(has_flag(type, remove_exceptions_typest::remove_added_instanceof))
    INVARIANT(
      false,
