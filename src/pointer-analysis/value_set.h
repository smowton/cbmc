...
public:
  ...
  /// The function this value-set's location belongs to
  irep_idt function;
  ...
  /// Dynamic object identifier, consisting of a number (allocation site id)
  /// and a recency state (most recent allocation or otherwise)
  typedef std::pair<unsigned int, dynamic_object_exprt::recencyt>
    dynamic_object_idt;

  /// Set of dynamic object IDs, equivalent to a set of
  /// `dynamic_object_exprt`s with corresponding state. Used only in internal
  ...
  /// Outputs JSON representation of this value set
  /// \param ns: global namespace
  /// \return JSON representation
  jsont output_json(const namespacet &ns) const;
