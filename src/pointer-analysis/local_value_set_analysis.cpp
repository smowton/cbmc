  ...
  if(fun.instructions.size()!=0)
  {
    ...
    for(const auto& param : function_type.parameters())
    {
      if(param.type().id()==ID_pointer)
      {
        ...
        value_sett::entryt param_entry_blank(param_name,"");
        auto& param_entry=initial_state.get_entry(param_entry_blank, param.type(), ns);
        external_value_set_exprt param_var(param.type(),constant_exprt(param_name,string_typet()));
