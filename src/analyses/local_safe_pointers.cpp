...
/// If `expr` is of the form `x != nullptr`, return x. Otherwise return nil.
static exprt get_null_checked_expr(const exprt &expr)
{
...
get_conditional_checked_expr(const exprt &goto_guard)
{
  ...
  exprt checked_expr = get_null_checked_expr(normalized_guard);
  if(checked_expr.is_nil())
    return {};
  else
    ...
    case ASSUME:
      {
        exprt checked_expr;
        if(
          !(checked_expr = get_null_checked_expr(instruction.guard)).is_nil())
        {
          ...
          ++subexpr_it)
      {
        if(subexpr_it->id() == ID_dereference)
        {
          if(!first)
            out << ", ";
          first = true;
          out << from_expr(ns, "", subexpr_it->op0());
        }
      }
      out << "}";
    }
