...
overlay_map() : base_map(0),removed_key_valid(false) {}
  ...
  std::pair<localiter, bool> insert(
    const itervaltype& newkv)
  {
  ...
  class const_iterator : public std::iterator<std::forward_iterator_tag,const itervaltype>
  {
  ...
  public:
	...
	if(localmapit!=localmapend && (!basemapit.is_end()) && localmapit->first==basemapit->first)
	{
    ...
    typename std::iterator<std::forward_iterator_tag, const typename std::map<K, V>::value_type>::reference operator*() const
    {
      if(basemapitp)
      {
	auto& basemapit=*basemapitp;
    ...
    typename std::iterator<std::forward_iterator_tag, const typename std::map<K, V>::value_type>::pointer operator->() const
    {
      return &**this;
    }
    ...
    void skip_removed_keys()
    {
      assert(basemapitp);
      auto& basemapit=*basemapitp;
      while(removekeyvalid && (!basemapit.is_end()) && basemapit->first==removekey)
	++basemapit;
    }
  ...
  const_iterator local_begin() const { return const_iterator(local_map.begin(),local_map.end()); }

  const_iterator cend() const { return const_iterator(true); }
  const_iterator end() const { return cend(); }
  const_iterator local_end() const { return const_iterator(true); }
  ...
  bool operator==(const overlay_map& other) const
  {
    ...
    const_iterator thisit=begin(),otherit=other.begin(),thisend=end(),otherend=other.end();    
