  ...
  goto_programt::targett target)
{
  ...
  *target =
  ...
  // fields (if the given type is void then the caller must set these up itself;
  // this happens in array[reference].clone(), where the type info must be
  // copied over from the input array)
  if(
    java_array_element_type(object_type) !=
    java_reference_type(java_void_type()))
  {
    const auto underlying_type_and_dimension =
      java_array_dimension_and_element_type(object_type);
    if(underlying_type_and_dimension.second >= 1)
    {
      ...
      dest.insert_before(
        ...
        goto_programt::make_assignment(code_assignt(
          ...
          from_integer(
            underlying_type_and_dimension.second,
            object_array_dimension.type()),
          location)));
