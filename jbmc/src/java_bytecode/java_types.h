...
public:
  ...
  enum class method_handle_typet
  {
   ...
   {
   public:
     java_lambda_method_handlet(const irep_idt &id,
       method_handle_typet handle_type) :
       irept(id)
     {
       set(ID_handle_type, (int)handle_type);
     }

     java_lambda_method_handlet() {
       set(ID_handle_type, (int)method_handle_typet::UNKNOWN_HANDLE);
     }
     ...
     method_handle_typet get_handle_type()
     const {
